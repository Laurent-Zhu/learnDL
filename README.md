...
- [x] 第十三章介绍了计算机视觉，介绍了一些目标检测算法，如SSD、R-CNN、Fast R-CNN、Faster R-CNN、YOLO，还有语义分割及全卷积网络FCN，还介绍了风格迁移

  - [x] **锚框**
    - [x] 假设输入图像的高度为 ℎ ，宽度为 𝑤，锚框由中心点、宽高比、缩放比决定
    - [x] 在训练集中，我们将每个锚框视为一个训练样本。 为了训练目标检测模型。
    - [x] 我们需要每个锚框的**类别（class）**和**偏移量（offset）**标签，其中前者是与锚框相关的对象的类别，后者是真实边界框相对于锚框的偏移量。如果一个锚框没有被分配真实边界框，我们只需将锚框的类别标记为背景（background）。
    - [x] 背景类别的锚框通常被称为负类锚框，其余的被称为正类锚框。
    - [x] 假设一个锚框 𝐴 被分配了一个真实边界框 𝐵 。 一方面，锚框 𝐴 的类别将被标记为与 𝐵 相同。 另一方面，锚框 𝐴 的偏移量将根据 𝐵 和 𝐴 中心坐标的相对位置以及这两个框的相对大小进行标记。

  - [x] `SSD`单发多框检测
    - [x] 单发多框检测是一种多尺度目标检测模型
    - [x] 单发多框检测生成不同数量和不同大小的锚框，并通过预测这些锚框的类别和偏移量检测不同大小的目标
    - [x] 损失函数是根据锚框的类别和偏移量的预测及标注值计算得出的
    - [x] 架构
      - [x] ![image-20250728175154323](assets/image-20250728175154323.png)

  - [x] `R-CNN`系列
    - [x] `R-CNN`（每个锚框都要通过一个卷积层）
      - [x] <img src="assets/image-20250728175357556.png" alt="image-20250728175357556" style="zoom:50%;" />

    - [x] `Fast R-CNN`（整张图片先过卷积层，然后再进行选择性搜索，再ROI池化）
      - [x] <img src="assets/image-20250728175445835.png" alt="image-20250728175445835" style="zoom:50%;" />

    - [x] `Faster R-CNN`
      - [x] <img src="assets/image-20250728175520537.png" alt="image-20250728175520537" style="zoom:50%;" />

    - [x] `Masked R-CNN`
      - [x] <img src="assets/image-20250728175603935.png" alt="image-20250728175603935" style="zoom:50%;" />

  - [x] 转置卷积
    - [x] 与通过卷积核减少输入元素的常规卷积**相反**，转置卷积通过卷积核**广播**输入元素，从而产生形状**大于**输入的输出。
    - [x] 如果我们将 𝖷输入卷积层 𝑓来获得输出 𝖸=𝑓(𝖷)并创造一个与 𝑓有相同的超参数、但输出通道数是 𝖷中通道数的转置卷积层 𝑔，那么 𝑔(𝑌)的形状将与 𝖷相同。
    - [x] 我们可以使用矩阵乘法来实现卷积。转置卷积层能够交换卷积层的正向传播函数和反向传播函数。
    - [x] <img src="assets/image-20250728175818053.png" alt="image-20250728175818053" style="zoom:50%;" />

  - [x] `FCN`全卷积网络
    - [x] 全卷积网络先使用卷积神经网络抽取图像特征，然后通过 1×1 卷积层将通道数变换为类别个数，最后通过转置卷积层将特征图的高和宽变换为输入图像的尺寸。
    - [x] <img src="assets/image-20250728175909331.png" alt="image-20250728175909331" style="zoom:67%;" />

  - [x] 风格迁移
    - [x] <img src="assets/image-20250728175937583.png" alt="image-20250728175937583" style="zoom:50%;" />
    - [x] 风格迁移常用的损失函数由3部分组成：
      - [x] （1）内容损失使合成图像与内容图像在内容特征上接近；
      - [x] （2）风格损失令合成图像与风格图像在风格特征上接近；
      - [x] （3）全变分损失则有助于减少合成图像中的噪点。

    - [x] 可以通过预训练的卷积神经网络来抽取图像的特征，并通过最小化损失函数来不断更新合成图像来作为模型参数。
    - [x] 使用格拉姆矩阵表达风格层输出的风格。
